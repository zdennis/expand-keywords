...
module ExpandKeyword
  class CLI
    ...
    def run_expand(argv)
      ...
      OptionParser.new do |o|
        o.on("--file PATH", "Path to keywords JSON file") { |v| options[:file] = v }
        o.on("-n NAMESPACE", "--namespace NAMESPACE", "Resolve colon-less tokens as \$NAMESPACE:token first, falling back to the plain token") { |v| options[:namespace] = v }
      ...
      <<~USAGE
        ...
        Subcommands:
          ...
          expand  [--file PATH] "text with $keywords"    Expand keywords in text
                  [-n NAMESPACE]                         Resolve colon-less tokens as $NAMESPACE:token first
